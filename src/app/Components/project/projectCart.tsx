...
export function ProjectCart({ data }: { data: cartType }) {
            ...
            <div>
                {projectDetailDisplay ? (
                    <>
                        {/* Lớp phủ nền màu xám */}
                        <div
                            className="fixed inset-0 bg-gray-500 opacity-[0.3] z-[4]"
                            onClick={() => setProjectDetailDisplay(false)}
                        />

                        {/* Nội dung div nằm giữa màn hình */}
                        <div className="flex justify-center items-center w-[80vw] max-w-[70rem] h-[85vh] lg:h-[50vw] max-h-[37rem] fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white text-black shadow-lg rounded-[20px] overflow-hidden z-20 border-[2px] border-[var(--text-color-gray)]">
